...
func Init(sharedCfg *sharedconfig.Shared, flagsets FlagSets) error {
	...
	appcfg.AddPFlag(fs.Serve, &shared.AuthAddrHost, "nw_track.host", "host to serve")
	appcfg.AddPFlag(fs.Serve, &shared.AuthAddrHostInt, "nw_track.host-int", "internal host to serve")
	appcfg.AddPFlag(fs.Serve, &shared.AuthAddrPort, "nw_track.port", "port to serve monolith introspection")
