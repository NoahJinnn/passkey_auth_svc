...
func (s *webauthnSuite) TestWebauthnHandler_BeginRegistration() {
	...
	if s.Equal(http.StatusOK, rec.Code) {
		...
		s.Equal(defaultCfg.Webauthn.RelyingParty.Id, creationOptions.Response.RelyingParty.ID)
...
func (s *webauthnSuite) TestWebauthnHandler_FinishRegistration() {
	if testing.Short() {
		s.T().Skip("skipping test in short mode")
	}
...
func (s *webauthnSuite) TestWebauthnHandler_BeginLogin() {
	if testing.Short() {
		s.T().Skip("skipping test in short mode")
	}
	...
	if s.Equal(http.StatusOK, rec.Code) {
		...
		s.Equal(defaultCfg.Webauthn.RelyingParty.Id, assertionOptions.Response.RelyingPartyID)
	}
}

func (s *webauthnSuite) TestWebauthnHandler_FinishLogin() {
	if testing.Short() {
		s.T().Skip("skipping test in short mode")
	}
