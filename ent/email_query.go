...
func (eq *EmailQuery) QueryPasscodes() *PasscodeQuery {
	query := (&PasscodeClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EmailQuery) QueryPrimaryEmail() *PrimaryEmailQuery {
	query := (&PrimaryEmailClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EmailQuery) WithPasscodes(opts ...func(*PasscodeQuery)) *EmailQuery {
	query := (&PasscodeClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withPasscodes = query
	return eq
}
...
func (eq *EmailQuery) WithPrimaryEmail(opts ...func(*PrimaryEmailQuery)) *EmailQuery {
	query := (&PrimaryEmailClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withPrimaryEmail = query
	return eq
}
...
func (eq *EmailQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Email, error) {
	...
	if query := eq.withPasscodes; query != nil {
		if err := eq.loadPasscodes(ctx, query, nodes,
			func(n *Email) { n.Edges.Passcodes = []*Passcode{} },
			func(n *Email, e *Passcode) { n.Edges.Passcodes = append(n.Edges.Passcodes, e) }); err != nil {
			return nil, err
		}
	}
	if query := eq.withPrimaryEmail; query != nil {
		if err := eq.loadPrimaryEmail(ctx, query, nodes, nil,
			func(n *Email, e *PrimaryEmail) { n.Edges.PrimaryEmail = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EmailQuery) loadPasscodes(ctx context.Context, query *PasscodeQuery, nodes []*Email, init func(*Email), assign func(*Email, *Passcode)) error {
...
func (eq *EmailQuery) loadPrimaryEmail(ctx context.Context, query *PrimaryEmailQuery, nodes []*Email, init func(*Email), assign func(*Email, *PrimaryEmail)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Email)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
