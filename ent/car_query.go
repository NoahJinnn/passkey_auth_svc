...
func (cq *CarQuery) FirstID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (cq *CarQuery) FirstIDX(ctx context.Context) uint {
...
func (cq *CarQuery) OnlyID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (cq *CarQuery) OnlyIDX(ctx context.Context) uint {
...
func (cq *CarQuery) IDs(ctx context.Context) (ids []uint, err error) {
...
func (cq *CarQuery) IDsX(ctx context.Context) []uint {
	ids, err := cq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (cq *CarQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Car, error) {
	...
	if query := cq.withUser; query != nil {
		if err := cq.loadUser(ctx, query, nodes, nil,
			func(n *Car, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CarQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Car, init func(*Car), assign func(*Car, *User)) error {
	ids := make([]uint, 0, len(nodes))
	nodeids := make(map[uint][]*Car)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (cq *CarQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(car.Table, car.Columns, sqlgraph.NewFieldSpec(car.FieldID, field.TypeUint))
	_spec.From = cq.sql
