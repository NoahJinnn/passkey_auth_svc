...
func (ac *AssetCreate) SetNillableUserID(u *uuid.UUID) *AssetCreate {
	if u != nil {
		ac.SetUserID(*u)
	}
	return ac
...
func (ac *AssetCreate) SetProviderName(s string) *AssetCreate {
	ac.mutation.SetProviderName(s)
	return ac
}

// SetNillableProviderName sets the "provider_name" field if the given value is not nil.
func (ac *AssetCreate) SetNillableProviderName(s *string) *AssetCreate {
	if s != nil {
...
func (ac *AssetCreate) defaults() {
	if _, ok := ac.mutation.Sheet(); !ok {
		v := asset.DefaultSheet
		ac.mutation.SetSheet(v)
	}
	if _, ok := ac.mutation.Section(); !ok {
		v := asset.DefaultSection
		ac.mutation.SetSection(v)
	}
...
func (ac *AssetCreate) check() error {
	...
	if _, ok := ac.mutation.ProviderName(); !ok {
		return &ValidationError{Name: "provider_name", err: errors.New(`ent: missing required field "Asset.provider_name"`)}
	}
	if _, ok := ac.mutation.Currency(); !ok {
		return &ValidationError{Name: "currency", err: errors.New(`ent: missing required field "Asset.currency"`)}
	}
	if _, ok := ac.mutation.Value(); !ok {
		return &ValidationError{Name: "value", err: errors.New(`ent: missing required field "Asset.value"`)}
	}
	if _, ok := ac.mutation.CreatedAt(); !ok {
