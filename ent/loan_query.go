...
func (lq *LoanQuery) FirstID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (lq *LoanQuery) FirstIDX(ctx context.Context) uint {
...
func (lq *LoanQuery) OnlyID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (lq *LoanQuery) OnlyIDX(ctx context.Context) uint {
...
func (lq *LoanQuery) IDs(ctx context.Context) (ids []uint, err error) {
...
func (lq *LoanQuery) IDsX(ctx context.Context) []uint {
	ids, err := lq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (lq *LoanQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Loan, error) {
	...
	if query := lq.withUser; query != nil {
		if err := lq.loadUser(ctx, query, nodes, nil,
			func(n *Loan, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (lq *LoanQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Loan, init func(*Loan), assign func(*Loan, *User)) error {
	ids := make([]uint, 0, len(nodes))
	nodeids := make(map[uint][]*Loan)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (lq *LoanQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(loan.Table, loan.Columns, sqlgraph.NewFieldSpec(loan.FieldID, field.TypeUint))
	_spec.From = lq.sql
