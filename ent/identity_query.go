...
func (iq *IdentityQuery) QueryEmail() *EmailQuery {
	query := (&EmailClient{config: iq.config}).Query()
...
func (iq *IdentityQuery) WithEmail(opts ...func(*EmailQuery)) *IdentityQuery {
	query := (&EmailClient{config: iq.config}).Query()
...
//		GroupBy(identity.FieldProviderID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Identity.Query().
//		Select(identity.FieldProviderID).
//		Scan(ctx, &v)
...
func (iq *IdentityQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Identity, error) {
	...
	if query := iq.withEmail; query != nil {
		if err := iq.loadEmail(ctx, query, nodes, nil,
			func(n *Identity, e *Email) { n.Edges.Email = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *IdentityQuery) loadEmail(ctx context.Context, query *EmailQuery, nodes []*Identity, init func(*Identity), assign func(*Identity, *Email)) error {
