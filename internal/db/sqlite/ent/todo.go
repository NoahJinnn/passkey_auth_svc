...
type Todo struct {
	...
	// ListId holds the value of the "listId" field.
	...
	for i := range columns {
		...
		case todo.FieldListId:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field listId", values[i])
			} else if value.Valid {
				t.ListId = int(value.Int64)
			}
		case todo.FieldText:
...
func (t *Todo) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", t.ListId))
	builder.WriteString(", ")
