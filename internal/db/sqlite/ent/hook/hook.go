...
// The ManualItemFunc type is an adapter to allow the use of ordinary
// function as ManualItem mutator.
type ManualItemFunc func(context.Context, *ent.ManualItemMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ManualItemFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ManualItemMutation); ok {
		return f(ctx, mv)
	}
