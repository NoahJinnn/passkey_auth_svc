...
func TestManager_GenerateJWT_IssAndAud(t *testing.T) {
	...
	cfg := sharedconfig.Session{
		...
		Lifespan: "5m",
		Audience: []string{"test.passkey_auth_svc.com"},
	}
	...
	assert.NoError(t, err)
	assert.Equal(t, []string{"test.passkey_auth_svc.com"}, token.Audience())
	assert.Equal(t, "passkey-auth-service", token.Issuer())
}
...
func TestManager_GenerateJWT_AdditionalAudiences(t *testing.T) {
	...
	cfg := sharedconfig.Session{
		...
		Lifespan: "5m",
		Audience: []string{
			"additional.passkey_auth_svc.com",
			"anotherOne",
		},
	}
...
func TestGenerator_Verify(t *testing.T) {
	sessionLifespan := "5m"
	manager := JwkManager{}
	cfg := sharedconfig.Session{Lifespan: sessionLifespan}
	sessionGenerator, err := session.NewManager(&manager, cfg)
	assert.NoError(t, err)
	require.NotEmpty(t, sessionGenerator)
...
func TestGenerator_DeleteCookie(t *testing.T) {
	...
	assert.Equal(t, "passkey-auth-service", cookie.Name)
}
